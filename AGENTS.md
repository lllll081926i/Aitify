...
- `src/src/watch.rs`: core watcher logic for Claude/Codex/Pi/OpenCode and detection rules.
...
- PRs should include:
  ...
  - impacted watcher source(s) (`claude`/`codex`/`pi`/`opencode`),
  - validation commands and results,
  - screenshots for UI changes (`ui/`) when applicable.

## Security & Configuration Tips
- Do not commit machine-specific data or session logs.
- Use env vars for tuning watcher behavior (for example `CODEX_TOKEN_GRACE_MS`, `CODEX_SEED_CATCHUP_MS`, `PI_FOLLOW_TOP_N`, `WATCH_CONFIRM_ALERT_ENABLED`).
